...
export class DbStorage implements IStorage {
  ...
  async updateTechnician(id: string, technician: Partial<InsertTechnician> & { isActive?: boolean }): Promise<Technician | undefined> {
    const result = await db.update(technicians).set(technician).where(eq(technicians.id, id)).returning();
    return result[0];
  }
  ...
  async updateTeam(id: string, team: Partial<InsertTeam> & { isActive?: boolean }): Promise<Team | undefined> {
  ...
  async getServiceOrderByCode(code: string): Promise<ServiceOrder | undefined> {
    const result = await db.select().from(serviceOrders).where(eq(serviceOrders.code, code)).limit(1);
    return result[0];
  }
  ...
  async createServiceOrder(serviceOrder: InsertServiceOrder): Promise<ServiceOrder> {
    const result = await db.insert(serviceOrders).values(serviceOrder).returning();
    return result[0];
  }

  async updateServiceOrder(id: string, serviceOrder: Partial<InsertServiceOrder>): Promise<ServiceOrder | undefined> {
    const result = await db.update(serviceOrders).set(serviceOrder).where(eq(serviceOrders.id, id)).returning();
    return result[0];
  }
  ...
  async updateNeighborhood(id: string, neighborhood: Partial<InsertNeighborhood>): Promise<Neighborhood | undefined> {
    const result = await db.update(neighborhoods).set(neighborhood).where(eq(neighborhoods.id, id)).returning();
    return result[0];
  }
  ...
  async createServiceType(serviceType: InsertServiceType): Promise<ServiceType> {
    const result = await db.insert(serviceTypes).values(serviceType).returning();
    return result[0];
  }

  async updateServiceType(id: string, serviceType: Partial<InsertServiceType>): Promise<ServiceType | undefined> {
    const result = await db.update(serviceTypes).set(serviceType).where(eq(serviceTypes.id, id)).returning();
    return result[0];
  }
